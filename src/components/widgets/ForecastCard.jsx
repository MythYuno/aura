  ...
  return (
    <Card padding="md" className="col-span-full" delay={0.1}>
      <div className="flex items-center gap-2 mb-3">
        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--ok-10)' }}>
          <Target size={14} className="text-ok" />
        </div>
      ...
      {forecast && (
        <p className="text-sm text-fg-2 leading-relaxed mb-4">
          A questo ritmo spenderai <strong className={cn('text-fg-1 font-semibold px-1.5 rounded tnum', forecast.willOverrun ? 'bg-red/12 text-red' : 'bg-ok/12 text-ok', privacy && 'privacy-blur')}>€{$d(forecast.projected)}</strong>
          {forecast.willOverrun
            ...
            : <> — risparmiando <strong className={cn('font-semibold tnum', privacy && 'privacy-blur')} style={{ color: 'var(--ok)' }}>€{$d(Math.abs(forecast.overrun))}</strong></>
          }
        </p>
      )}
      ...
      {/* Progress bar */}
      <div className="flex items-center gap-2 mb-3">
        <div className="flex-1 h-1.5 bg-bg-2 rounded-full overflow-hidden border border-bd-1">
          <div
            className="h-full rounded-full transition-all duration-1000"
            style={{
              width: `${burnPct}%`,
              background: isOver ? 'linear-gradient(90deg, var(--red), #FF4444)' : 'linear-gradient(90deg, var(--ok), var(--info))',
              boxShadow: isOver ? '0 0 10px rgba(255,90,90,.4)' : '0 0 10px var(--accent-glow)',
            }}
          />
        </div>
        <span className="text-[11px] font-bold tnum" style={{ color: isOver ? 'var(--red)' : 'var(--ok)' }}>
          {Math.round(burnPct)}%
        </span>
      </div>
      ...
      <div className="flex gap-4 mt-1">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-0.5 rounded-full bg-ok opacity-50" />
