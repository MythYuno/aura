...
export const SpliceModal = ({ open, amount, label, onConfirm, onClose }) => {
  ...
  // v0.8.1: BUGFIX. Prima `if (!open) return null` smontava il componente
  // brutalmente — exit animation di AnimatePresence non veniva mai eseguita.
  // Ora la condizione è dentro AnimatePresence per scale-out fluido.

  // months è bloccato dal slider in [2, 24] ma defendiamo anche da manomissioni
  // (devtools, prop esterno bug). Clamp e fallback per evitare Infinity nei calcoli.
  const safeMonths = Number.isFinite(months) && months > 0 ? Math.min(24, Math.max(1, months)) : 1;
  const perMonth = amount / safeMonths;
