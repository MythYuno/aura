...
export const registerSW = (onUpdate) => {
  ...
  window.addEventListener('load', async () => {
    ...
    // Salvato in window per poter eventualmente fermare il polling dall'esterno
    // (es. su Capacitor iOS dove il service worker non serve).
    const POLL = 30 * 60 * 1000;
    const pollId = setInterval(() => { reg.update().catch(() => {}); }, POLL);
    ...
    // Cleanup hook: chiamando window.__auraStopSW() (es. da Capacitor bridge)
    // fermiamo polling e listener. Niente più job pendenti.
